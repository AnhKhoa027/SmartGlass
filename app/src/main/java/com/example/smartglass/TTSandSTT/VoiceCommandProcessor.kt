...
class VoiceCommandProcessor(
    ...
    init {
        // Theo dõi các thay đổi trong cài đặt
        scope.launch {
            settings.volumeFlow.collect { /* Có thể đọc phản hồi âm lượng nếu cần */ }
        }
        scope.launch {
            settings.speedFlow.collect { /* Có thể đọc phản hồi tốc độ nếu cần */ }
        }
        scope.launch {
            settings.keepScreenOnFlow.collect { /* Có thể phản hồi trạng thái màn hình */ }
        }
    }
    ...
    fun handleCommand(command: String): Boolean {
        ...
        return when {
                ...
                if (isConnected) {
                    ...
                    onDisconnect { success ->
                        if (success) {
                            ...
                            voiceResponder("Hủy kết nối thất bại, thử lại nhé.")
                        }
                    }
                } else {
                    voiceResponder("Thiết bị chưa được kết nối.")
                }
                true
            }
            ...
            cmd.contains("kết nối") -> {
                if (!isConnected) {
                    ...
                    onConnect { success ->
                        if (success) {
                            isConnected = true
                            voiceResponder("Đã kết nối thiết bị thành công.")
                        } else {
                            voiceResponder("Kết nối thất bại, vui lòng thử lại.")
                        }
                    }
                } else {
                    voiceResponder("Thiết bị đã được kết nối sẵn rồi.")
                }
                true
            }
            ...
            // ------------------ MIC / LẮNG NGHE ------------------
            cmd.contains("tắt mic") || cmd.contains("dừng lại") || cmd.contains("ngưng lắng nghe") -> {
                voiceResponder("Sẽ tắt mic và dừng lắng nghe.")
                true
            }
            ...
            cmd.contains("hiện tại") && (cmd.contains("cả") || cmd.contains("tất cả")) -> {
                voiceResponder(
                    "Âm lượng bây giờ là ${settings.getVolume()}%, " +
                            "tốc độ đọc ${speedText(settings.getSpeed())}, " +
                            "và màn hình ${if (settings.isKeepScreenOn()) "đang bật" else "có thể tự khóa"}."
                )
                true
            ...
            // ------------------ CÀI ĐẶT ÂM LƯỢNG ------------------
            cmd.contains("tăng âm lượng") -> {
                adjustVolume(up = true)
                true
            }

            cmd.contains("giảm âm lượng") -> {
            ...
            cmd.contains("đặt âm lượng") -> {
                extractNumber(cmd)?.let {
                    settings.setVolume(it.coerceIn(0, 100))
                    voiceResponder("Đã đặt âm lượng thành ${it.coerceIn(0, 100)}%")
                } ?: voiceResponder("Không nghe rõ mức âm lượng bạn muốn đặt.")
                true
            }

            // ------------------ CÀI ĐẶT TỐC ĐỘ ------------------
            cmd.contains("tăng tốc độ đọc") -> {
                adjustSpeed(up = true)
                true
            }

            cmd.contains("giảm tốc độ đọc") -> {
                adjustSpeed(up = false)
                true
            }
            ...
            // ------------------ MÀN HÌNH ------------------
            cmd.contains("bật màn hình") || cmd.contains("sáng màn hình") || cmd.contains("không khóa màn hình") -> {
                settings.setKeepScreenOn(true)
                voiceResponder("Màn hình sẽ luôn sáng.")
                true
            }

            cmd.contains("tắt màn hình") || cmd.contains("khóa màn hình") || cmd.contains("không sáng màn hình") -> {
                settings.setKeepScreenOn(false)
                voiceResponder("Thiết bị có thể tự khóa màn hình.")
                true
            }
