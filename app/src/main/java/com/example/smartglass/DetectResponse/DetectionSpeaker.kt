...
class DetectionSpeaker(
    ...
    //Update người dùng di chuyển
    private fun updateUserMovement() {
        lastSensorDirY = sensorBuffer.getUserMoveDirY()
        lastSensorDirX = sensorBuffer.getUserMoveDirX()
    }
    ...
    fun speakDetections(
        ...
        if (lastSpokenKey == key) {
            Log.d("TTS_SKIP", "Skip same object $key")
            return
        }
        ...
        VisionContext.update(
            listOf(
                VisionContext.SeenObject(
                    name = box.clsName,
                    direction = normalizedDir,
                    distance = if (distanceText.isNotEmpty())
                        distanceText else "không xác định",
                    movement = relation
                )
            )
        )
