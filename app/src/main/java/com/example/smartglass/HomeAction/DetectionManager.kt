...
class DetectionManager(
    ...
    private suspend fun processObjectDeepCheck(trackedObj: TrackedObject) {
        ...
        val crop = cropBoundingBox(frame, trackedObj.smoothBox) ?: return
        ...
        try {
            ...
            if (customConf >= THRESH_CUSTOM_HIGH) {
                finalBox = finalBox.copy(
                    clsName = customLabel,
                    cnf = customConf,
                    boxColor = Color.BLUE,
                    source = DetectSource.CLASSIFIER
                )
                updated = true
            } else if (bestYolo != null && bestYolo.cnf >= THRESH_YOLO_HIGH) {
                finalBox = finalBox.copy(
                    clsName = bestYolo.clsName,
                    cnf = bestYolo.cnf,
                    boxColor = Color.GREEN,
                    source = DetectSource.YOLO
                )
                updated = true
            } else if (bestYolo != null && bestYolo.cnf >= THRESH_CONSENSUS &&
                customConf >= THRESH_CONSENSUS &&
                areLabelsSimilar(bestYolo.clsName, customLabel)
            ) {
                // Consensus YOLO + Classifier
                finalBox = finalBox.copy(
                    clsName = customLabel,
            ...
            if (updated) {
                trackedObj.uncertainSince = 0L
                trackedObj.isUnknown = false
                trackedObj.smoothBox = finalBox
                withContext(Dispatchers.Main) {
                    cameraViewManager.setOverlayResults(
                        currentTrackedObjects.map { it.smoothBox }
                    )
                    detectionSpeaker.speakDetections(
                        listOf(trackedObj),
                        cameraViewManager.getOverlayWidth(),
            ...
            if (!updated) {
                if (trackedObj.uncertainSince == 0L) {
                    trackedObj.uncertainSince = now
                }

                val duration = now - trackedObj.uncertainSince

                if (duration >= 2000 && !trackedObj.isUnknown) {
                    ...
                    try {
                        ...
                        if (apiResults.isNotEmpty()) {
                            ...
                            if (bestApi.score >= THRESH_API_HIGH ||
                                ...
                                trackedObj.smoothBox = trackedObj.smoothBox.copy(
                                    clsName = LabelTranslator.toVietnamese(bestApi.label),
                                    cnf = bestApi.score,
                                    boxColor = Color.RED,
                                    source = DetectSource.API
                                )
